...
<script>
import { getPersonalDetailApi, getJobDetailApi, getUserDetailByIdApi } from '@/api/employees'
export default {
  data() {
    return {
      formData: {}
  ...
  methods: {
    async getPersonalDetail() {
      const { data: userInfo } = await getUserDetailByIdApi(this.userId) // 获取个人基本信息(顶部)
      const { data: detail } = await getPersonalDetailApi(this.userId) // 获取个人基本信息(底部)
      this.formData = { ...detail, ...userInfo }
    },
    async getJobDetail() {
      const { data: userInfo } = await getUserDetailByIdApi(this.userId)
      const { data: jobInfo } = await getJobDetailApi(this.userId) // 获取个人基本信息
